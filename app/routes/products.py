from fastapi import APIRouter, HTTPException, status, Query, Depends, Request, UploadFile, File, Form
from fastapi.security import HTTPBearer, HTTPAuthorizationCredentials
from app.models.products import (
    ProductCreate, ProductUpdate, ProductResponse, ProductListItem,
    ProductsListResponse, ProductSearchFilters, FeaturedProductsResponse,
    ProductStats, ProductSortBy, ProductCondition, SupportedCurrencies,
    SellerInfo, CategoryInfo, SubcategoryInfo
)
...
async def get_featured_products(
    ...
    try:
        ...
        """).eq("featured", True).gt("quantity", 0)
        ...
        sellers_info = {}
        if seller_ids:
            sellers_response = supabase.table("users").select("user_id, name, business_name").in_("user_id", seller_ids).execute()
            for seller in sellers_response.data:
                sellers_info[seller["user_id"]] = seller["business_name"] or seller["name"]

        categories_info = {}
        if category_ids:
            categories_response = supabase.table("categories").select("id, name").in_("id", category_ids).execute()
            for category in categories_response.data:
                categories_info[category["id"]] = category["name"]

        subcategories_info = {}
        if subcategory_ids:
            subcategories_response = supabase.table("subcategories").select("id, name").in_("id", subcategory_ids).execute()
            for subcategory in subcategories_response.data:
                subcategories_info[subcategory["id"]] = subcategory["name"]

        featured_products = []
        for product in response.data:
            featured_products.append(ProductListItem(
                id=product["id"],
                name=product["name"],
                price=product["price"],
        ...
        return FeaturedProductsResponse(
            ...
            total_count=len(featured_products)
        )
...
async def get_my_products(
    ...
    sort_by: ProductSortBy = Query(ProductSortBy.CREATED_AT, description="Sort products by"),
    ...
    try:
        ...
        # Get total count
        count_response = supabase.table("products").select("id", count="exact").eq("sellerId", current_user["user_id"]).execute()
        ...
        products_list = []
        for product in response.data:
            products_list.append(ProductListItem(
                id=product["id"],
                name=product["name"],
                price=product["price"],
                currency=product["currency"],
...
async def get_product_stats(
    ...
    try:
        ...
        total_response = supabase.table("products").select("id", count="exact").execute()
        total_products = total_response.count or 0

        # Get products by category
        categories_response = supabase.table("products").select("categoryId, categories(name)").execute()
        products_by_category = {}
        for item in categories_response.data:
            category_name = item.get("categories", {}).get("name", "Unknown")
            products_by_category[category_name] = products_by_category.get(category_name, 0) + 1
        ...
        for item in conditions_response.data:
            ...
            products_by_condition[condition] = products_by_condition.get(condition, 0) + 1
        ...
        featured_response = supabase.table("products").select("id", count="exact").eq("featured", True).execute()
        featured_products_count = featured_response.count or 0

        # Get available products count
        available_response = supabase.table("products").select("id", count="exact").gt("quantity", 0).execute()
...
async def get_products(
    ...
    currency: Optional[SupportedCurrencies] = Query(None, description="Filter by currency"),
    ...
    search_query: Optional[str] = Query(None, description="Search in product names and descriptions"),
    current_user = Depends(get_current_user)
):
    ...
    try:
        ...
        if search_query:
            count_response = count_response.or_(f"name.ilike.%{search_query}%,description.ilike.%{search_query}%")
...
async def get_product_by_id(
    ...
    include_category_info: bool = Query(True, description="Include category information"),
    current_user = Depends(get_current_user)
):
    ...
    try:
        ...
        if include_seller_info:
            seller_response = supabase.table("users").select("user_id, name, email, phone_number, business_name").eq("user_id", product["sellerId"]).execute()
        ...
        if include_category_info:
            category_response = supabase.table("categories").select("id, name, description").eq("id", product["categoryId"]).execute()
            ...
            subcategory_response = supabase.table("subcategories").select("id, name, description").eq("id", product["subCategoryId"]).execute()
...
async def create_product(
    ...
    try:
        ...
        category_response = supabase.table("categories").select("id").eq("id", product.categoryId).execute()
        ...
        subcategory_response = supabase.table("subcategories").select("id, category_id").eq("id", product.subCategoryId).execute()
        ...
        return ProductResponse(
            ...
            allowPurchaseOnPlatform=created_product.get("allowPurchaseOnPlatform", False),
...
@router.post("/products/create-with-images", response_model=ProductResponse, status_code=status.HTTP_201_CREATED)
async def create_product_with_images(
    ...
    try:
        ...
        # Verify category and subcategory exist
        category_response = supabase.table("categories").select("id").eq("id", categoryId).execute()
        if not category_response.data or len(category_response.data) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid category ID"
        ...
        subcategory_response = supabase.table("subcategories").select("id, category_id").eq("id", subCategoryId).execute()
        if not subcategory_response.data or len(subcategory_response.data) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid subcategory ID"
            ...
            for file in images:
                ...
                try:
                    ...
                    for uploaded_url in uploaded_urls:
                        try:
                            path = uploaded_url.split("/")[-2] + "/" + uploaded_url.split("/")[-1]
...
async def update_product(
    ...
    try:
        ...
        existing_product_response = supabase.table("products").select("*").eq("id", product_id).execute()

        if not existing_product_response.data or len(existing_product_response.data) == 0:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Product not found"
            )
        ...
        if product.categoryId or product.subCategoryId:
            ...
            subcategory_response = supabase.table("subcategories").select("id, category_id").eq("id", subcategory_id).execute()
            if not subcategory_response.data or len(subcategory_response.data) == 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Invalid subcategory ID"
        ...
        return ProductResponse(
            ...
            allowPurchaseOnPlatform=updated_product.get("allowPurchaseOnPlatform", False),
...
async def delete_product(
    ...
    try:
        # Check if product exists and belongs to the current user
        existing_product_response = supabase.table("products").select("sellerId").eq("id", product_id).execute()
...
async def toggle_product_featured(
    ...
    try:
        # Check if product exists and belongs to the current user
        existing_product_response = supabase.table("products").select("sellerId, featured").eq("id", product_id).execute()
        ...
        logger.error(f"Error toggling featured status for product {product_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def toggle_online_payment(
    ...
    try:
        # Check if product exists and belongs to the current user
        existing_product_response = supabase.table("products").select("sellerId, allowPurchaseOnPlatform").eq("id", product_id).execute()
        ...
        logger.error(f"Error toggling online payment for product {product_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
